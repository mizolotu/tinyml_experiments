...
if __name__ == '__main__':
    ...
    parser = arp.ArgumentParser(description='Train auto (Bayes search) regression model to detect anomalous vibrations.')
    ...
    for board in args.boards:
        ...
        model_fname = f'{board}_{args.algorithm}_auto_ray'
        ...
        def train_cs_model(config, data):
        ...
        def train_skm_model(config, data):
        ...
        def train_som_model(config, data):
        ...
        def train_dsvdd_model(config, data):
            ...
            progress.append(f'{acc},{fpr},{tpr},{auc},{config["n_layers"]},{config["n_units"]},{config["n_outputs"]}\n')
        ...
        if args.algorithm == 'cs':
            hp_keys = ['config/n_clusters', 'config/p_microclusters']
        elif args.algorithm == 'skm':
            hp_keys = ['config/n_clusters', 'config/p_batch_size']
        elif args.algorithm == 'som':
            hp_keys = ['config/map_height', 'config/map_width']
        elif args.algorithm == 'dsvdd':
            hp_keys = ['config/n_layers', 'config/n_units', 'config/n_outputs']
        else:
            raise NotImplemented

        hps = df[hp_keys].values

        print(len(accs), len(fprs), len(tprs), len(aucs), len(hps))

        progress = []
        for i, (acc, fpr, tpr, auc) in enumerate(zip(accs, fprs, tprs, aucs)):
            hp_line = ','.join([str(hp) for hp in hps[i, :]])
            progress.append(f'{acc},{fpr},{tpr},{auc},{hp_line}\n')
