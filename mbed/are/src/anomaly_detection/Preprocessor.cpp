...
Preprocessor::Preprocessor() {
    ...
    fv_q = DynamicDimensionQueue(fv_q_size, fft_features * x_dim);
    ...
    feature_vector_ = new float[fft_features * x_dim];
...
Preprocessor::Preprocessor(bool _use_fft) {
...
bool Preprocessor::put(float *x) {
    ...
    if (is_baseline_calculated) {
        ...
        x_d = 0.0;
        for (unsigned short i=0; i < x_dim; i++) {
            x_d += pow(x_std_frozen[i] > eps ? (x[i] - x_mean[i]) / x_std_frozen[i] : 0.0, 2);
        	//x_d += pow(x_std[i] > eps ? (x[i] - x_mean[i]) / x_std[i] : 0.0, 2);
        }
        	...
        	if (f_step >= fft_step) {
        		...
        		if (use_fft) {
        			...
        			for (unsigned short j = 0; j < x_dim; j++) {
        				for (unsigned short i = 0; i < fft_step; i++) {
        					...
        					re[i] = (x_std_frozen[j] <= eps) ? 0.0 : (x2[j] - x_mean[j]) / x_std_frozen[j];
        					//re[i] = (x_std[j] <= eps) ? 0.0 : (x2[j] - x_mean[j]) / x_std[j];
        					im[i] = 0;
        				}
        			...
        			for (unsigned short j = 0; j < x_dim; j++) {
        				for (unsigned short i = 0; i < fft_features; i++) {
        					x_q.get(x_q.size() - fft_features + i, x2);
        			        re[i] = (x_std_frozen[j] <= eps) ? 0.0 : (x2[j] - x_mean[j]) / x_std_frozen[j];
        			    }
            	...
            	if (is_feature_std_found) {
            		...
            		*/
            		...
            		fv_q.enqueue(feature_vector);

            		if (batch_mode) {

            			fv_count += 1;
            			if (fv_count == fv_q_size) {
            				is_ready = true;
            				fv_count = 0;
            				f_step = 0;
            			}
        ...
        if (x_step == baseline_steps) {
            ...
            for (unsigned short i=1; i < x_q.size(); i++) {
                x_q.get(i, x);
                    x_d = 0.0;
                    for (unsigned short j=0; j < x_dim; j++) {
                        x_d += pow(x_std_frozen[j] > eps ? (x[j] - x_mean[j]) / x_std_frozen[j] : 0.0, 2);
                    }
                    x_d = maximum(0.0, pow(x_d, 0.5));
                    x_d_s += x_d;
                    x_d_ss += pow(x_d, 2);
...
float* Preprocessor::get_feature_vector(short i) {
	fv_q.get(i, feature_vector_);
...
short Preprocessor::get_batch_size() {
	return fv_q_size;
}

bool Preprocessor::get_batch_mode() {
	return batch_mode;
}

void Preprocessor::set_batch_mode(bool bm) {
	batch_mode = bm;
}
