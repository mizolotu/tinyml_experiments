...
#lte_table = 'WindsideHourData'
lte_table = 'Windside10minData'
...
lte_start_date_col = 'StartDate'
lte_start_timestamp_col = 'StartTimestamp'
lte_end_date_col = 'EndDate'
lte_end_timestamp_col = 'EndTimestamp'
lte_last_id_col = 'LastId'
...
def generate_html(url, params):
    ...
    if sensor == 'lte':
        cols, rows = _select_samples_between_lte(sdate=sdate, edate=edate)
    elif sensor == 'lora':
    ...
    if len(rows) > 0:
        ...
        dates, timestamps, currents, voltages, powers, windspeeds, sdate_, edate_ = prepare_series(sensor, cols, rows, sdate, edate)
        #logging.info('PREPARED!')
        
        #logging.info('Preparing hists...')
        hist_lines = prepare_hists(sensor, cols, rows)
        #logging.info('PREPARED!')
...
def prepare_series(sensor, cols, rows, sdate, edate, ts_step=60*10):

    if sensor == 'lte':
    
        start_dates = np.array([row[cols.index(lte_start_date_col)] for row in rows])
        start_timestamps = [row[cols.index(lte_start_timestamp_col)] for row in rows]
        end_dates = [row[cols.index(lte_end_date_col)] for row in rows]
        end_timestamps = [row[cols.index(lte_end_timestamp_col)] for row in rows]
        
        sdate_ = start_dates[0] if sdate is None else sdate
        edate_ = start_dates[-1] if edate is None else edate
        
        sample_ns = [row[cols.index(lte_cols[0])] for row in rows]

        avg_currents_ = np.array([row[cols.index(lte_cols[1])] for row in rows])
        min_currents_ = np.array([row[cols.index(lte_cols[2])] for row in rows])
        max_currents_ = np.array([row[cols.index(lte_cols[3])] for row in rows])

        avg_voltages_ = np.array([row[cols.index(lte_cols[4])] for row in rows])
        min_voltages_ = np.array([row[cols.index(lte_cols[5])] for row in rows])
        max_voltages_ = np.array([row[cols.index(lte_cols[6])] for row in rows])

        avg_powers_ = np.array([row[cols.index(lte_cols[7])] for row in rows])
        min_powers_ = np.array([row[cols.index(lte_cols[8])] for row in rows])
        max_powers_ = np.array([row[cols.index(lte_cols[9])] for row in rows])

        avg_windspeeds_ = np.array([row[cols.index(lte_cols[10])] for row in rows])
        min_windspeeds_ = np.array([row[cols.index(lte_cols[11])] for row in rows])
        max_windspeeds_ = np.array([row[cols.index(lte_cols[12])] for row in rows])

        #logging.info(sdate)
        #logging.info(edate)

        start_timestamps_u, start_timestamps_u_idx = np.unique(start_timestamps, return_index=True)
        n_u = len(start_timestamps_u)

        currents = {'avg': np.zeros(n_u), 'min': np.zeros(n_u), 'max': np.zeros(n_u)}
        voltages = {'avg': np.zeros(n_u), 'min': np.zeros(n_u), 'max': np.zeros(n_u)}
        powers = {'avg': np.zeros(n_u), 'min': np.zeros(n_u), 'max': np.zeros(n_u)}
        windspeeds = {'avg': np.zeros(n_u), 'min': np.zeros(n_u), 'max': np.zeros(n_u)}
        
        for i, start_timestamps_u_i in enumerate(start_timestamps_u_idx):
            #logging.info(f'{i + 1} / {n_u}')
            
            #idx = np.where(start_timestamps == start_timestamps_u[i])[0]
            #logging.info(idx)
            
            if i < n_u - 1:
                idx = np.arange(start_timestamps_u_i, start_timestamps_u_idx[i + 1])
            else:
                idx = n_u - 1
            #logging.info(idx)
            
            currents['avg'][i] = np.mean(avg_currents_[idx])
            currents['min'][i] = np.min(min_currents_[idx])
            currents['max'][i] = np.max(max_currents_[idx])

            voltages['avg'][i] = np.mean(avg_voltages_[idx])
            voltages['min'][i] = np.min(min_voltages_[idx])
            voltages['max'][i] = np.max(max_voltages_[idx])

            powers['avg'][i] = np.mean(avg_powers_[idx])
            powers['min'][i] = np.min(min_powers_[idx])
            powers['max'][i] = np.max(max_powers_[idx])
           
            windspeeds['avg'][i] = np.mean(avg_windspeeds_[idx])
            windspeeds['min'][i] = np.min(min_windspeeds_[idx])
            windspeeds['max'][i] = np.max(max_windspeeds_[idx])
...
def prepare_hists(sensor, cols, rows, ws_step=0.1):
    ...
    if sensor == 'lte':
        ...
        currents = {
            'avg': np.array([row[cols.index(lte_cols[1])] for row in rows]),
            'min': np.array([row[cols.index(lte_cols[2])] for row in rows]),
            'max': np.array([row[cols.index(lte_cols[3])] for row in rows])
        }
    
        voltages = {
            'avg': np.array([row[cols.index(lte_cols[4])] for row in rows]),
            'min': np.array([row[cols.index(lte_cols[5])] for row in rows]),
            'max': np.array([row[cols.index(lte_cols[6])] for row in rows])
        }

        windspeeds = {
            'avg': np.array([row[cols.index(lte_cols[10])] for row in rows]),
            'min': np.array([row[cols.index(lte_cols[11])] for row in rows]),
            'max': np.array([row[cols.index(lte_cols[12])] for row in rows])
        }
...
def _select_samples_between_lte(sdate=None, edate=None):

    rows = []
    #zone = 'Europe/Helsinki'
    #zone = 'UTC'

    query = [f"select * from {lte_table}"]
    if sdate is not None or edate is not None:
        query.append(' where ')
        if sdate is not None:
            #sdate_ut = pytz.timezone('UTC').localize(datetime.strptime(sdate, '%d/%m/%Y %H:%M:%S')).astimezone(pytz.timezone('Europe/Helsinki')).timestamp()
            #sdate_ut = datetime.strptime(sdate, '%d/%m/%Y %H:%M:%S').astimezone(pytz.timezone(zone)).timestamp()
            sdate_ut = hlk_tz.localize(datetime.strptime(sdate, '%d/%m/%Y %H:%M:%S')).timestamp()
            #logging.info(sdate_ut)
            query.append(f'{lte_start_timestamp_col} >= {sdate_ut}')
            if edate is not None:
                #edate_ut = datetime.strptime(edate, '%d/%m/%Y %H:%M:%S').astimezone(pytz.timezone(zone)).timestamp()
                edate_ut = hlk_tz.localize(datetime.strptime(edate, '%d/%m/%Y %H:%M:%S')).timestamp()
                #logging.info(edate_ut)
                query.append(f' AND {lte_start_timestamp_col} <= {edate_ut}')
        else:
            #edate_ut = datetime.strptime(edate, '%d/%m/%Y %H:%M:%S').astimezone(pytz.timezone(zone)).timestamp()
            edate_ut = hlk_tz.localize(datetime.strptime(edate, '%d/%m/%Y %H:%M:%S')).timestamp()
            query.append(f'{lte_start_timestamp_col} <= {edate_ut}')
    query.append(f' order by {lte_start_timestamp_col}')
