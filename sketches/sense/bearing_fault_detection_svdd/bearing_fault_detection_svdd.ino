  ...
  while(1) {
    ...
    if (status) {
      ...
      if (t - t_stage_start <= d_train) {
        ...
        // on lora, can we also send these two values (model.get_n_train() and model.get_n_val()), to make sure that the model has been trained and vaidated properly?
...
void _inference() {
  ...
  while(1) {
    ...
    if (status) {      
      ...
      if (score_thr > 0) {
        ...
        score_to_thr_ratio += maximum(0.0, score / score_thr - 1);
        n_scores += 1; 
  ...
  // on lora, substitute the text below with the code to send the average score to the threshold ratio and/or other information, e.g. timestamp?
  //--------------------------------------------------------------------------------
