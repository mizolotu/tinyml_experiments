...
arduinoFFT::arduinoFFT(double *vReal, double *vImag, unsigned short samples, double samplingFrequency) {
  this->_vReal = vReal;
  this->_vImag = vImag;
...
unsigned char arduinoFFT::Revision(void) {
  return(FFT_LIB_REV);
}

void arduinoFFT::Compute(double *vReal, double *vImag, unsigned short samples, unsigned char dir) {
  //#warning("This method is deprecated and may be removed on future revisions.")
  Compute(vReal, vImag, samples, Exponent(samples), dir);
}

void arduinoFFT::Compute(unsigned char dir) {
  ...
  double c2 = 0.0;
  unsigned short l2 = 1;
  for (unsigned char l = 0; (l < this->_power); l++) {
...
void arduinoFFT::Compute(double *vReal, double *vImag, unsigned short samples, unsigned char power, unsigned char dir) {
  ...
  //#warning("This method is deprecated and may be removed on future revisions.")
  unsigned short j = 0;
  for (unsigned short i = 0; i < (samples - 1); i++) {
    if (i < j) {
      Swap(&vReal[i], &vReal[j]);
      if(dir==FFT_REVERSE)
        Swap(&vImag[i], &vImag[j]);
    }
    unsigned short k = (samples >> 1);
    while (k <= j) {
      j -= k;
      k >>= 1;
    }
  ...
  double c2 = 0.0;
  unsigned short l2 = 1;
  for (unsigned char l = 0; (l < power); l++) {
    unsigned short l1 = l2;
    l2 <<= 1;
    double u1 = 1.0;
    double u2 = 0.0;
    for (j = 0; j < l1; j++) {
       for (unsigned short i = j; i < samples; i += l2) {
          unsigned short i1 = i + l1;
...
void arduinoFFT::ComplexToMagnitude(double *vReal, double *vImag, unsigned short samples) {
...
void arduinoFFT::DCRemoval(double *vData, unsigned short samples) {
...
void arduinoFFT::Windowing(unsigned char windowType, unsigned char dir) {
...
void arduinoFFT::Windowing(double *vData, unsigned short samples, unsigned char windowType, unsigned char dir) {
...
double arduinoFFT::MajorPeak(double *vD, unsigned short samples, double samplingFrequency) {
  ...
  for (unsigned short i = 1; i < ((samples >> 1) + 1); i++) {
...
void arduinoFFT::MajorPeak(double *vD, unsigned short samples, double samplingFrequency, double *f, double *v) {
  //#warning("This method is deprecated and may be removed on future revisions.")
...
unsigned char arduinoFFT::Exponent(unsigned short value) {
